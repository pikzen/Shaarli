...
function checkUpdate()
{
    ...
    if (version_compare($newestversion, shaarli_version) == 1) {
...
if (isset($_POST['login'])) {
    ...
    if (isset($_POST['password']) && tokenOk($_POST['token']) && (check_auth($_POST['login'], $_POST['password']))) {   // Login/password is OK.
        ...
        if (isset($_GET['post'])) {
            header('Location: ?post='.urlencode($_GET['post']).(!empty($_GET['title']) ? '&title='.urlencode($_GET['title']) : '').(!empty($_GET['description']) ? '&description='.urlencode($_GET['description']) : '').(!empty($_GET['source']) ? '&source='.urlencode($_GET['source']) : ''));
            exit;
        }
        ...
        $redir = '';
        if (isset($_GET['post'])) {
            $redir = '&post='.urlencode($_GET['post']).(!empty($_GET['title']) ? '&title='.urlencode($_GET['title']) : '').(!empty($_GET['description']) ? '&description='.urlencode($_GET['description']) : '').(!empty($_GET['source']) ? '&source='.urlencode($_GET['source']) : '');
        }
...
function getHTTP($url, $timeout = 30)
{
    try {
        ...
        // getHTTP *can* fail silently (we don't care if the title cannot be fetched)
...
function genThumbnail()
{
    ...
    if ($domain == 'flickr.com' || endsWith($domain, '.flickr.com')) {
        ...
        $imageurl = '';
        if (endswith(parse_url($url, PHP_URL_PATH), '.jpg')) {  // This is a direct link to an image. e.g. http://farm1.staticflickr.com/5/5921913_ac83ed27bd_o.jpg
